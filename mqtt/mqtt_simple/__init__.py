...
class MqttClient: 
    ...
    def __init__(self, loop, log, callback = None):
        ...
        self.topicPubUpdate    = "1/MC30AEA4D7D4D0/{}_{}/state"
        self.mqttLive = False
    ...
    def _subscribeTopic(self):
        try:
            ...
            self.client.subscribe(topic = self.topicSubOperation.format(self.honyar_sn, 1), qos = 0)
            self.client.subscribe(topic = self.topicSubOperation.format(self.honyar_sn, 2), qos = 0)
            self.client.subscribe(topic = self.topicSubOperation.format(self.honyar_sn, 3), qos = 0)
    ...
    def publishMsg(self, ieeeAddr, index, msg):
        try:
            if ieeeAddr == self.HONYAR_ID:
                ...
                topic = "{}/state".format(self.door_sn)
                self.client.publish(topic = topic, msg = msg, qos = 0)
            elif ieeeAddr == self.MOTION_ID:
                topic = "{}/state".format(self.MOTION_ID)
                self.client.publish(topic = topic, msg = msg, qos = 0)
        except Exception as e:
...
class App:
    ...
    def onMsgReceived(self, topic, msg):
        # topic[1/MC30AEA4D7D4D0/3163294935805788_1/set
    ...
    async def taskDataReceived(self, n):
        ...
        if n.SrcAddr == HONYAR_NETWORK_ADDRESS:  # Javis Switch
            index = n.SrcEndpoint
            if n.Data[6] == 0:
                ...
                if 'state' not in list_nodes[ieeeAddr]:
                    ...
                    list_nodes[ieeeAddr]['state']["switch" + str(index)] = "off"
            elif n.Data[6] == 1:
